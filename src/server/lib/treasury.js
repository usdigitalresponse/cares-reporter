...
async function createOutputWorkbook (
  ...
  for (let i = 0; i < wbSpec.settings.length; i++) {
    ...
    switch (outputSheetName) {
      ...
      case 'Sub Recipient':
        objSR = getSubRecipientSheet(sheetRecords, outputColumnNames, period_id)
...
/*  getSubRecipientSheet() converts the subrecipient records from postgres
  into a pair of output arrays
  the ddRecords were generated by deDuplicate() from the subrecipients table
  ...
  */
function getSubRecipientSheet (ddRecords, outputColumnNames, period_id) {
  ...
  ddRecords.forEach(ddRecord => {
    ...
    // on SAM.gov."
    if (jsonRow[dunsSourceName] && ddRecord.referenced) {
